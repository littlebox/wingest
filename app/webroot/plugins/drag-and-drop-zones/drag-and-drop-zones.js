...
/*
Preparing data to be saved Original
*/
...
/*
Preparing data to be saved Modified (Genera el string JSON para parsear en el controlador)
*/
function saveDragDropNodes()
{
	var saveString = "";
	saveString += '[';
	var uls = dragDropTopContainer.getElementsByTagName('UL');
	for(var no=1;no<uls.length;no++){	// Looping through all <ul> except the one who contain all items
		saveString += '{"Zone":{"id":' + uls[no].id + '},"Team":{"Team":[';
		var lis = uls[no].getElementsByTagName('LI');
		for(var no2=0;no2<lis.length;no2++){
			saveString += lis[no2].id;
			if(no2<(lis.length-1)) saveString += ',';
		}
		saveString += ']}}';
		if(no<(uls.length-1)) saveString += ',';
	}
	saveString += ']';
